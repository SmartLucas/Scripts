...
function telaCheia(){
	$('#playback-container').css({"width":	$('#room').width(),
				      "height":	$('#room').height(),
				      "left":	"0%",
				      "z-index":"7"});
				      
	$('#playback').css({"width":	$('#room').width(),
			    "height":	$('#room').height(),
			    "left":	"0%",
			    "z-index":	"8"});
...
function chatCom(value) {
	if ( value.trim() == "/fullon" ) {
		API.chatLog('Painel de vídeo em ' +
			    $('#room').width() +
			    'x' +
			    $('#room').height(),
		false);
	...
	if ( value.trim() == "/fulloff" ) {
		...
		API.chatLog('Painel de vídeo restaurado para ' +
			    playback_style.width.substring(0,playback_style.width.length-2) +
			    'x' +
			    playback_container.height.substring(0,playback_container.height.length-2),
		false);
	}
}
